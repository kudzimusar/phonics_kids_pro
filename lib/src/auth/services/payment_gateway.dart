...
class PaymentGateway {
  ...
  Future<void> initiateSubscription({required String email, required String tierId}) async {
    try {
      ...
      print("Stripe Payment Sheet closed. Check Firestore for subscription status.");
      
    } catch (e) {
      if (e is StripeException) {
        print("Error from Stripe: ${e.error.localizedMessage}");
      } else if (e is FirebaseFunctionsException) {
        print("Error from Cloud Functions: [${e.code}] ${e.message}");
      } else {
        print("Unforeseen error in PaymentGateway: $e");
      }
      rethrow;
    }
  }
